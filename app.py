...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'secfrom flask import Flask, render_template, jsonify
from flask_socketio import SocketIO, emit
import json
import random
from threading import Timer
...
def siradaki_oyuncuya_gec():
    ...
    socketio.emit('yeni_tur', {
        ...
        'kalan_sure': SURE_SINIRI,
        'tur_sayisi': oyun['tur_sayisi'],
        'bomba_sayaci': 2,  # veya belirlediğiniz sayı
        'puanlar': oyun['puanlar']
    })
...
if __name__ == '__main__':
    ...
    # Production ortamı için (Railway)
        ...
        socketio.run(app, host='0.0.0.0', port=port) ret!'
socketio = SocketIO(app, async_mode='eventlet', cors_allowed_origins='*')
